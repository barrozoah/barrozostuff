...
! *** local variables
integer                                         ::      i,j,i3,j3,mol,ic,nits
real(8)                                         ::      length,diff,corr,scp,xxij2
real(8)                                         ::      xij(3),xxij(3)
#if defined (PROFILING)
real(8)                                         :: start_loop_time
start_loop_time = rtime()
        ...
        do !iteration loop
                do ic=1,shake_mol(mol)%nconstraints
                        ...
                        if (.not. shake_mol(mol)%bond(ic)%ready) then
                                ...
                                i3 = i*3-3
                                j3 = j*3-3
                                Bij(k+1)  = x(i3+1) - x(j3+1)
                                Bij(k+2)  = x(i3+2) - x(j3+2)
                                Bij(k+3)  = x(i3+3) - x(j3+3)
                                length    = sqrt(Bij(k+1)**2+Bij(k+2)**2+Bij(k+3)**2)
                                Bij(k+1)  = Bij(k+1) / length
                                Bij(k+2)  = Bij(k+2) / length
                                Bij(k+3)  = Bij(k+3) / length
                end do
                do ic=1,shake_mol(mol)%nconstraints
                                diff    = shake_mol(mol)%bond(ic)%dist2 - xij2
                                if(abs(diff) < SHAKE_TOL*shake_mol(mol)%bond(ic)%dist2) then
